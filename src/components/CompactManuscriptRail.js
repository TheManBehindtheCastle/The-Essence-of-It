...
const CompactManuscriptRail = React.memo(({ items, onPress }) => {
  ...
  return (
    <View style={styles.wrapper}>
      ...
      >
        {items.map((item, i) => (
          <TouchableOpacity key={i} onPress={() => onPress(item)} style={[styles.card, { width: WIDTH }]}>
      ...
      <InteractiveDots total={items.length} active={active} onPagePress={(p) => scrollRef.current?.scrollTo({ x: p * SNAP, animated: true })} />
    </View>
  );
});
